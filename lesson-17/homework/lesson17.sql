...
-----------------------------
-- 1. Distributors and Sales by Region
-----------------------------
...
-----------------------------
-- 3. Products with at least 100 units in Feb 2020
-----------------------------
...
SELECT s.Device_id,
       ...
       MAX(CASE WHEN r.rnk = 1 THEN r.Locations END) AS max_signal_location,
       SUM(s.cnt) AS no_of_signals
FROM SignalCount s
JOIN Ranked r ON s.Device_id = r.Device_id AND s.Locations = r.Locations
GROUP BY s.Device_id;
------------------------------------------------------------


-----------------------------
-- 7. Employees earning more than dept avg
-----------------------------
DROP TABLE IF EXISTS Employee;
...
FROM (
    SELECT t.TicketID,
           COUNT(DISTINCT CASE WHEN n.Number IS NOT NULL THEN t.Number END) AS cnt,
           (SELECT COUNT(*) FROM Numbers) AS total
    FROM Tickets t
...
FROM (
  SELECT Spend_date, User_id, 'Mobile' AS Platform, Mobile_Amt AS Total_Amount
  FROM base WHERE Mobile_Amt > 0
  UNION ALL
  SELECT Spend_date, User_id, 'Desktop' AS Platform, Desktop_Amt AS Total_Amount
  FROM base WHERE Desktop_Amt > 0
  UNION ALL
  SELECT Spend_date, User_id, 'Both' AS Platform, (Mobile_Amt + Desktop_Amt) AS Total_Amount
  FROM base WHERE Mobile_Amt > 0 AND Desktop_Amt > 0
) t
